...
function(Layer, Annotator, util) {
  ...
  function createMainDisplay(params, data, annotator, imageLayer) {
    ...
    displayContainer.className = "disp-container";
    imageContainer.appendChild(displayContainer);
    displayContainer.appendChild(imageLayer.canvas);
  ...
  function createSidebar(params, data, annotator) {
    ...
    exportButton.addEventListener("click", function () {
      ...
      // var annotationURL = getCurrentAnnoImageURL(params);
      // var filename = annotationURL.split(/[\\/]/).pop();
      //
      //
  ...
  function getCurrentImageURL(params){
    return `data/images/${params.scan}/imgs/ser002img000${params.id}.dcm.jpeg`;
  }
  function getScanLength(params){
    return annotationInfo.slideIndicator.length;
  }
  function getNextId(params){
    ...
    return next.substring(next.length-2);
  }
  function getPrevId(params){
    var id = parseInt(params.id);
    id--;
    var next = "0"+id;
    return next.substring(next.length-2);
  }
  function isAnnotated(params){
    if(params.annotated != undefined){
      return true;
    }else{
      return false;
    }
  }
  function getCurrentAnnoImageURL(params){
    return `data/annotations/${params.scan}/ser002img000${params.id}.png`;
  }

  // Entry point.
  function render(data, params) {
    var imageURL = getCurrentImageURL(params);

    var id = parseInt(params.id, 10);
    if (isNaN(id))
      throw("Invalid id");
    var annotator = new Annotator(imageURL, {
          width: params.width,
        ...
        imageLayer = new Layer(getCurrentImageURL(params), {
          width: params.width,
          height: params.height
        });
